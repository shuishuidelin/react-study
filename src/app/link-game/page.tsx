        ...
        <div className="flex justify-around">
            <div className="w-5/12 flex flex-col">
                {wordData.slice(0,wordData.length/2).map((item) =>{
                    return (<TextBrick text={item.question} key={item.id} action={{state:item.state}} onChoose={()=>onChooseWord(item.id)}></TextBrick>)
                })}
            </div>
            <div className="w-5/12 flex flex-col">
                {wordData.slice(wordData.length/2).map((item) =>{
                    return (<TextBrick text={item.answer} key={item.id}  action={{state:item.state}}  onChoose={()=>onChooseWord(item.id)}></TextBrick>)
                })}
            </div>
        </div>
