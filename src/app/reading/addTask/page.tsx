...
import { TaskStep, PreTask } from "@/app/reading/interfaces"
import { addTask } from "@/app/reading/server"
import {
  Dialog,
  DialogActions,
  DialogContent,
...
const AddTaskPage = () => {
  ...
  const requestServerData = async () => {
    return new Promise<TaskStep[]>((resolve) => {
      setTimeout(() => {
  ...
  const generateTasks = async (data: TaskStep[]) => {
    const newTask: PreTask = {
      steps: data.map((step: TaskStep, index: number) => ({
    ...
    if (result.id) setNewTaskId(result.id)
  ...
  return (
    <div>
      ...
      <Dialog open={successDialog} onClose={handleCloseDialog}>
        ...
        <DialogContent>
          <DialogContentText>
            The task has been added successfully. Do you want to go to the new
            task?
          </DialogContentText>
        </DialogContent>
        <DialogActions>
          ...
          <Button onClick={handleGoToTask} color="primary" autoFocus>
            Go to Task
          </Button>
        </DialogActions>
      </Dialog>
